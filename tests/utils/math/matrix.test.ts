...
describe("Matrix4", () => {
  ...
  it("should compute the matrix inverse correctly", () => {
    ...
    const inverseMatrix = Matrix4.identity();
    ...
    const expectedInverse = new Matrix4([
      ...
      new Vector([0, -0.375, 0.3125, 0.1875]),
      new Vector([
        -0.1052631578947368421, -0.054824561403508771915,
        0.042763157894736836, 0.26425438596491224,
      ]),
    ]);
  ...
  it("should compute scalar multication correctly", () => {
    ...
    const scaledM = Matrix4.identity();
    m.scalarMultiply(scaledM, 5);
    const expected = new Matrix4([
      new Vector([5 * 0.194, 5 * 1.536, 5 * 2.001, 5 * 4.448]),
      new Vector([5 *1.616, 5 * 2.976, 5 * 7.069, 5 * 0.557]),
      new Vector([5* 4.625, 5 * 3.461, 5 * 1.169, 5 * 8.455]),
      new Vector([5* 7.024, 5 * 4.827, 5 * 6.737, 5 * 7.44]),
    ]);
    expect(expected).toDeepEqual(expected);
  });
});
